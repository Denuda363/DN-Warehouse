  ...
  return (
    <div className="space-y-6 max-w-4xl mx-auto pb-8">
      <div>
        ...
        <p className="text-slate-500 text-sm">Kelola konfigurasi sistem Anda.</p>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          ...
          <CardContent className="space-y-6">
            ...
            <div>
              <p className="text-sm font-medium mb-3 flex items-center">
                <Palette className="w-4 h-4 mr-2 text-slate-500" /> Warna Utama
              </p>
                ...
                {colors.map(c => (
                  <button
                    ...
                    className={`w-10 h-10 rounded-full flex items-center justify-center transition-all ${c.bgClass} ${
                      data.colorTheme === c.id ? 'ring-4 ring-slate-300 dark:ring-slate-700 scale-110 shadow-lg' : 'hover:scale-105 hover:shadow opacity-80'
                    }`}
                  />
                ))}
              </div>
            ...
            <div className="pt-4 border-t border-slate-200 dark:border-slate-800 space-y-4">
               <div className="flex items-center gap-2 mb-4">
                 <input 
                   ...
                   onChange={(e) => updateData({ navIsTransparent: e.target.checked })}
                   className="w-4 h-4 rounded border-slate-300 text-indigo-600 focus:ring-indigo-500"
                 />
                 <label htmlFor="navIsTransparent" className="text-sm font-medium">Navigasi Transparan</label>
               </div>
               
               {!data.navIsTransparent && (
                 <div>
                   <p className="text-sm font-medium mb-1">Warna Background Navigasi</p>
                   <div className="flex gap-2">
                     <Input 
                       type="color" 
                       value={data.navBgColor || '#0f172a'} 
                       onChange={(e) => updateData({ navBgColor: e.target.value })}
               ...
               <div>
                 <p className="text-sm font-medium mb-1">Warna Teks Navigasi</p>
                 <div className="flex gap-2">
                   <Input 
                     type="color" 
                     value={data.navTextColor || '#e2e8f0'} 
                     onChange={(e) => updateData({ navTextColor: e.target.value })}
                     className="w-12 p-1 h-9 cursor-pointer"
                   />
                   <Input 
                     type="text" 
               ...
               <div className="flex justify-start">
                 <Button variant="outline" size="sm" onClick={() => updateData({ navBgColor: '#0f172a', navTextColor: '#e2e8f0' })}>
                   Reset ke Default
                 </Button>
               </div>
            </div>
        ...
        <Card>
          ...
          <CardContent className="space-y-4">
            <Button onClick={handleBackup} className="w-full mb-2 border border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900 text-slate-900 dark:text-slate-100 hover:bg-slate-50 dark:hover:bg-slate-800">
            ...
            <p className="text-xs text-slate-500 text-center mt-2">Simpan data secara berkala untuk menghindari kehilangan log stok.</p>
          </CardContent>
        </Card>
          ...
          <CardContent className="space-y-4">
            <div>
              ...
              <Input
                ...
                onChange={(e) => updateData({ warehouseProfile: { ...data.warehouseProfile, name: e.target.value } as any })}
            ...
            <div>
              ...
              <Input
                ...
                onChange={(e) => updateData({ warehouseProfile: { ...data.warehouseProfile, address: e.target.value } as any })}
            ...
            <div>
              ...
              <Input
                ...
                onChange={(e) => updateData({ warehouseProfile: { ...data.warehouseProfile, phone: e.target.value } as any })}
            ...
            <div>
              ...
              <div className="flex items-center gap-4 mt-2">
                {data.warehouseProfile?.logo ? (
                  <img src={data.warehouseProfile.logo} alt="Logo" className="w-16 h-16 object-contain rounded border border-slate-200 p-1 bg-white" />
                ) : (
                ...
                <div>
                  <div className="relative">
                    <Button type="button" variant="outline" size="sm" className="relative w-32 border-slate-300">
                      ...
                      <input 
                        ...
                        onChange={(e) => {
                          ...
                          if (file) {
                            const reader = new FileReader();
                            reader.onload = (event) => {
                              updateData({ warehouseProfile: { ...data.warehouseProfile, logo: event.target?.result as string } as any });
                            };
                  ...
                  {data.warehouseProfile?.logo && (
                    <Button 
                      ...
                      onClick={() => updateData({ warehouseProfile: { ...data.warehouseProfile, logo: undefined } as any })}
                    >
                      Hapus Logo
                    </Button>
                  )}
