...
export const calculateReturnTotal = (inv: PurchaseInvoice) => {
  ...
  const returnedTaxAmount = inv.useTax ? (returnedTaxableAmount * inv.taxPercent) / 100 : 0;
...
export const PurchaseInvoiceList: React.FC<{ onEdit: (id: string) => void }> = ({ onEdit }) => {
  ...
  const handleCancel = (invoice: PurchaseInvoice) => {
    ...
    if (!confirm('Apakah Anda yakin ingin membatalkan faktur ini? Stok barang akan dikurangi kembali.')) return;
  ...
  const executeReturn = () => {
    ...
    if (qty > availableToReturn) {
      alert(`Jumlah return tidak boleh melebihi qty faktur (${availableToReturn})!`);
      return;
    }
                    ...
                    >
                      ...
                      <td className="px-4 py-3 text-right">
                        {hasReturn ? (
                          <div className="flex flex-col items-end">
                            <span className="font-mono text-xs line-through text-slate-400">Rp {new Intl.NumberFormat('id-ID').format(inv.total)}</span>
                            <span className="font-mono font-bold text-emerald-600 dark:text-emerald-400">Rp {new Intl.NumberFormat('id-ID').format(finalEffectiveTotal)}</span>
                          </div>
                        ) : (
                          <span className="font-mono font-bold text-emerald-600 dark:text-emerald-400">Rp {new Intl.NumberFormat('id-ID').format(inv.total)}</span>
                        )}
                      </td>
                      ...
                      <td className="px-4 py-3 text-right" onClick={(e) => e.stopPropagation()}>
                        <div className="flex items-center justify-end gap-1">
                          ...
                          {!isCanceled && (
                            <>
                              <Button variant="ghost" size="icon" onClick={() => onEdit(inv.id)} title="Edit Faktur" className="text-slate-500 hover:text-indigo-600 h-8 w-8">
                                <Edit className="w-4 h-4" />
                              </Button>
                              <Button variant="ghost" size="icon" onClick={() => handleCancel(inv)} title="Batalkan Faktur" className="text-slate-500 hover:text-orange-600 h-8 w-8">
                                <Ban className="w-4 h-4" />
                              </Button>
                            </>
                          )}
                          <Button variant="ghost" size="icon" onClick={() => handleDelete(inv.id)} title="Hapus Permanen" className="text-slate-500 hover:text-red-600 h-8 w-8">
                        ...
                        <td colSpan={6} className="px-0 py-0">
                          <div className="border-t dark:border-slate-800 bg-slate-50 dark:bg-slate-900 p-4 shadow-inner">
                            <h4 className="text-sm font-bold text-slate-800 dark:text-slate-200 mb-3 ml-2 flex items-center">
                              <FileText className="w-4 h-4 mr-2 text-slate-400" /> Detail Barang Faktur {inv.invoiceNo}
                            </h4>
                              ...
                              <table className="w-full text-xs text-left min-w-[600px]">
                                <thead className="bg-slate-100 dark:bg-slate-800 text-slate-500 uppercase">
                                  <tr>
                                    ...
                                    <th className="px-3 py-2 text-center">Batch / Exp</th>
                                    <th className="px-3 py-2 text-right">Harga</th>
                                    <th className="px-3 py-2 text-center">Qty Input</th>
                                    <th className="px-3 py-2 text-center">Retur</th>
                                ...
                                <tbody className="divide-y divide-slate-200 dark:divide-slate-700">
                                  {inv.items.map(item => {
                                    ...
                                    if(item.discountValue > 0) {
                                      const text = item.discountType === 'Rp' ? `Rp ${new Intl.NumberFormat('id-ID').format(item.discountValue)}` : `${item.discountValue}%`;
                                      discountComponent = <span className="block text-red-500 font-mono text-[10px] mt-0.5">Disc: {text}</span>
                                    }
                                    return (
                                      <tr key={item.id} className="bg-white dark:bg-slate-950">
                                        <td className="px-3 py-2 font-medium">{product?.name || 'Unknown'}</td>
                                        <td className="px-3 py-2 text-center text-slate-500">
                                          {item.batchNo || '-'} {item.expDate ? ` / ${item.expDate}` : ''}
                                        </td>
                            ...
                            <div className="mt-4 mr-2 flex justify-end">
                              <div className="text-xs text-slate-500 bg-white dark:bg-slate-950 px-4 py-2 rounded-lg border border-slate-200 dark:border-slate-700 inline-block shadow-sm">
                                <div className="grid grid-cols-2 gap-x-4 gap-y-1">
                                  <span className="text-slate-400 uppercase tracking-wider">Metode Bayar:</span>
                                  <span className="font-medium text-right text-slate-700 dark:text-slate-300">{inv.paymentMethod}</span>
                                  <span className="text-slate-400 uppercase tracking-wider">Diskon Global:</span>
                                  <span className="font-medium text-right text-red-500 italic">
                                    {inv.invoiceDiscValue > 0 ? (inv.invoiceDiscType === 'Rp' ? `Rp ${new Intl.NumberFormat('id-ID').format(inv.invoiceDiscValue)}` : `${inv.invoiceDiscValue}%`) : '-'}
                                  </span>
                                  {inv.useTax && (
                                    <>
                                      <span className="text-slate-400 uppercase tracking-wider">PPN:</span>
                                      <span className="font-medium text-right text-slate-700 dark:text-slate-300">{inv.taxPercent}%</span>
                                    </>
                                  )}
                                  <span className="text-slate-400 uppercase tracking-wider pt-2 mt-1 border-t dark:border-slate-800">Total Awal:</span>
                                  ...
                                  {hasReturn && (
                                    <>
                                      <span className="text-orange-500 font-bold uppercase tracking-wider">Total Retur:</span>
                                      <span className="font-bold text-right text-orange-500 text-sm">
                                        - Rp {new Intl.NumberFormat('id-ID').format(returnTotal)}
                                      </span>
                                      <span className="text-emerald-600 dark:text-emerald-400 font-bold uppercase tracking-wider pt-2 mt-1 border-t border-slate-300 dark:border-slate-700">Total Akhir:</span>
                                      <span className="font-bold text-right pt-2 mt-1 border-t border-slate-300 dark:border-slate-700 text-emerald-600 dark:text-emerald-400 text-sm">
                                        Rp {new Intl.NumberFormat('id-ID').format(finalEffectiveTotal)}
                                      </span>
                                    </>
                                  )}
                                </div>
          ...
          <Card className="w-full max-w-sm overflow-hidden animate-in fade-in zoom-in-95 duration-200">
            <div className="p-6">
              ...
              <p className="text-sm text-slate-500 mb-4">
                Masukkan jumlah barang yang akan dikembalikan ke supplier. Maksimal retur: <strong className="text-slate-800 dark:text-slate-200">{returnModal.maxQty}</strong>.
              </p>
              
              <div className="space-y-4">
                <div>
                  <label className="text-xs font-bold text-slate-500 uppercase tracking-wider mb-1 block">Qty Return</label>
                  <Input 
                    type="number"
                    min="1"
                ...
                <div className="flex gap-2 justify-end pt-2">
                  ...
                  <Button variant="primary" className="bg-orange-500 hover:bg-orange-600" onClick={executeReturn}>Proses Retur</Button>
                </div>
              </div>
            </div>
          </Card>
